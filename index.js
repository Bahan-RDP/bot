...
let db_respon_list = JSON.parse(fs.readFileSync('./database/bot/list-message.json'));
let db_dashboard = JSON.parse(fs.readFileSync('./database/bot/dashboard.json'));
let db_saldo = JSON.parse(fs.readFileSync("./database/bot/saldo.json"));
...
// Response
const { api, apikey, apikeyAntlatic, botName, owner, ownerNomer, ownerName, footer, packname, author, sessionName, thumbnail, qris, menfess, source, bc, responP, sp, mess, payment, host, application, serverCreate } = require("./options/setting");
...
const isOwner = ["628817839722","6285669029379",owner,ronzz.user.id.split('@')[0]].includes(sender.split('@')[0]) ? true : false
...
fs.writeFileSync(topupPath + sender.split("@")[0] + ".json", JSON.stringify(data_topup, null, 3))
ronzz.sendTextMentions(from, `「 *KONFIRMASI-TOPUP* 」
...
_Ketik *Y* untuk melanjutkan, *N* untuk mengulangi inputan_`, [data_topup.number])
...
_Jika sudah transfer, Silahkan kirim bukti transfer + nama pengirim agar bisa di proses oleh admin 🙏_
...
{title: "𝗛𝗼𝘀𝘁𝗶𝗻𝗴 / 𝗪𝗲𝗯𝗽",
rows: [
{title: "Webp", rowId: prefix+"webp", description: "Menampilkan list harga webp"},
]}
]
...
if (cekUser("id", sender) == null) return sendMessRegis(from)
const sections = [
{title: "𝗧𝗼𝗽𝘂𝗽 & 𝗩𝗼𝘂𝗰𝗵𝗲𝗿 𝗚𝗮𝗺𝗲",
rows: [
{title: "𝗚𝗮𝗿𝗲𝗻𝗮 𝗦𝗵𝗲𝗹𝗹 𝗠𝘂𝗿𝗮𝗵", rowId: prefix+"list_garena_shell", description: "Menampilkan list harga Voucher Garena Shell"},
{title: "𝗚𝗮𝗿𝗲𝗻𝗮 𝗙𝗿𝗲𝗲 𝗙𝗶𝗿𝗲", rowId: prefix+"list_diamond_ff", description: "Menampilkan list harga Diamond FF Murah"},
{title: "𝗠𝗼𝗯𝗶𝗹𝗲 𝗟𝗲𝗴𝗲𝗻𝗱𝘀", rowId: prefix+"list_diamond_ml", description: "Menampilkan list harga Diamond ML Murah"},
]},
{title: "𝗦𝗮𝗹𝗱𝗼 & 𝗧𝗼𝗸𝗲𝗻 𝗣𝗟𝗡",
rows: [
...
if ([`${ownerNomer}`,"628817839722","6285669029379"].includes(q)) return reply('Tidak boleh spam call ke owner.')
if (![`${ownerNomer}`,"628817839722","6285669029379"].includes(q)) {
...
if (cekUser("premium", sender) !== true) return reply(mess.prem)
if (!q) return reply(`Format salah ❌\n\nContoh :\n${prefix+command} 628817839722`)
if ([`${ownerNomer}`,"628817839722","6285669029379"].includes(q)) return reply('Tidak boleh spam sms ke owner.')
if (![`${ownerNomer}`,"628817839722","6285669029379"].includes(q)) {
...
case 'webp':
if (cekUser("id", sender) == null) return sendMessRegis(from)
if (!q) return ronzz.sendMessage(from, { text: `Silahkan kak @${sender.split("@")[0]} pencet button list message di bawah ya`, footer: footer, mentions: [sender], buttonText: 'Tampilan Webp', sections: [{title: "MAU TAMPILAN APA KAK?",rows:[{title: "MOBILE LEGEND", description: "Harga: Rp10000", rowId: prefix+"webp 1"},{title: "CODASHOP", description: "Harga: Rp10000", rowId: prefix+"webp 2"},{title: "FREEFIRE", description: "Harga: Rp10000", rowId: prefix+"webp 3"},{title: "SIMONTOK 18+", description: "Harga: Rp10000", rowId: prefix+"webp 4"},{title: "YOUTUBE 18+", description: "Harga: Rp10000", rowId: prefix+"webp 5"},{title: "GROUPWA 18+", description: "Harga: Rp10000", rowId: prefix+"webp 6"},{title: "STUMBLE CLAM", description: "Harga: Rp10000", rowId: prefix+"webp 7"},{title: "SAFELINKU 18+", description: "Harga: Rp10000", rowId: prefix+"webp 8"},{title: "XNXX 18+", description: "Harga: Rp10000", rowId: prefix+"webp 9"},{title: "VIDEO 18+", description: "Harga: Rp10000", rowId: prefix+"webp 10"},{title: "BANK BCA", description: "Harga: Rp10000", rowId: prefix+"webp 11"},{title: "BANK SYARIAH", description: "Harga: Rp10000", rowId: prefix+"webp 12"},{title: "BANK KREDIVO", description: "Harga: Rp10000", rowId: prefix+"webp 13"}]}], listType: 1 }, { quoted: msg })
if (/1/.test(q)) {
if (cekSaldo(sender ,db_saldo) < 10000) return reply(`Maaf *${cekUser("name", sender)}*, saldo kamu tidak mencukupi, Silahkan melakukan deposit terlebih dahulu sebelum menggunakan fitur silahkan ketik ${prefix}deposit untuk melakukan isi saldo`)
...
ronzz.sendMessage(sender,{text: `O━• 𝘽𝙀𝙍𝙃𝘼𝙎𝙄𝙇 𝙈𝙀𝙈𝘽𝙐𝘼𝙏 𝙒𝙀𝘽 •━O
...
addSaldo(sender, Number('10000'), db_saldo)
reply("*COBA LAGI DALAM 5 MENIT*") //error
}
});
}
if (/2/.test(q)) {
...
ronzz.sendMessage(`${ownerNomer}@s.whatsapp.net`, bukti_bayar)
...
let anu = await fetchJson('https://raw.githubusercontent.com/Ronzz-Ofc/database/master/games/tebakbendera.json')
let result = anu[Math.floor(Math.random() * anu.length)]
...
await sleep(50)
ronzz.sendTextMentions(from, `「 *SALDO USER* 」
⭔ID: ${q.split("|")[0]}
⭔Nomer: @${q.split("|")[0]}
⭔Tanggal: ${tanggal}
